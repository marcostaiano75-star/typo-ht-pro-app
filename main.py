...
def main(page: ft.Page):
    ...
    in_punta = ft.TextField(label="QUOTA PUNTA", expand=1, keyboard_type="number", color=color_accent, border_color=color_accent)
    in_banca = ft.TextField(label="QUOTA BANCA", expand=1, keyboard_type="number", color=color_error, border_color=color_error)
    ...
    def analizza_click(e):
        ...
        try:
            ...
            if dd_camp.value in df_bibbia.index and c_p in df_bibbia.columns:
                ...
                # LOGICA BINARIA (Entrambi devono essere OK)
                ok_punta = v_p < lim_p
                ok_banca = v_b > lim_b
                ...
                # Profitto Netto Punta (Back)
                prof_punta = (rischio_max * (v_p - 1)) * (1 - comm)
                
                # Profitto Netto Banca (Lay) -> Responsabilità = rischio_max
                ...
                if trade_on:
                    ...
                    col_res.controls.append(
                        ft.Container(
                            content=ft.Column([
                                ...
                                ft.Text(dettaglio_MM, textAlign="center"),
                                ft.Text(f"PROFITTO NETTO: € {profitto_migliore:.2f}", size=18, weight="bold", color=color_success)
                    ...
                    col_res.controls.append(
                        ft.Container(
                            ...
                            padding=15, bgcolor="#16213e", border_radius=12, alignment=ft.alignment.center
                        )
                    )
            else:
    ...
    page.add(
        ft.Column([
            ft.Row([ft.Icon(ft.Icons.ANALYTICS, color=color_cyan), ft.Text("TYPO HT-PRO MOBILE", size=20, weight="black")], alignment="center"),
